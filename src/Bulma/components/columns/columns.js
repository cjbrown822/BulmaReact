...
const ColumnsCore = withOptions(withGap(withDevice(styled.div.attrs(({gapStyle, deviceStyle, optionStyle}) => ({
	className: `columns${gapStyle ? ` ${gapStyle}` : ``}${deviceStyle ? ` ${deviceStyle}`:``}${optionStyle ? ` ${optionStyle}` : ``}`
}))``)));

const ColumnCore = withNarrow(withModifiers(styled.div.attrs(({modStyle, narrowStyle}) => ({
	className: `column ${modStyle}${narrowStyle ? ` ${narrowStyle}`:``}`
}))``));


const Column = props => <ColumnCore {...props}/>;

const Columns = props => <ColumnsCore {...props}/>;
